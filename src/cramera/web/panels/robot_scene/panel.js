...
Panels.define('robot-scene', function mountRobotScene(root, bus) {
  ...
  function applyLive(st) {
    ...
    for (const key in liveStateKeys) {
      if (key === liveDraggedKey && !(SCENE && SCENE.physics)) continue;
      const g = objectMeshes[key];
      if (g) {
  ...
  function renderFrame() {
    ...
    if (composer && ambientOcclusion) composer.render();
  ...
  RobotView.onReady(function () {
    if (SCENE.physics) {
      ...
      laboratoryPhysics = LaboratoryPhysics.mount(
        ...
        new LaboratoryPhysics.Client(window.fetch.bind(window), SCENE.physics), adapter,
      );
      ...
      if (!SCENE.physics.robot) {
        laboratoryRobotPhysicsEntry = LaboratoryPhysics.mountStart(
          ...
          new LaboratoryPhysics.Client(window.fetch.bind(window), {robot: true}),
    ...
    laboratoryWorkbench = LaboratoryWorkbench.mount(
      ...
      function (host) {
        const robot = LaboratoryRobot.mount(host, new LaboratoryRobot.Client(window.fetch.bind(window)));
        const physics = LaboratoryPhysics.mountStart(host, new LaboratoryPhysics.Client(window.fetch.bind(window)), function (url) { window.location.assign(url); });
        const robotPhysics = LaboratoryPhysics.mountStart(host, new LaboratoryPhysics.Client(window.fetch.bind(window), {robot: true}), function (url) { window.location.assign(url); });
