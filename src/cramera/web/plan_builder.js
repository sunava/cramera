...
(function () {
  ...
  function generateClass(stepsOverride) {
    ...
    L.push('');
    L.push('    # set while running under the cramera viewer, so build_context can use its ROS node');
    L.push('    visualization: WorldVisualization = field(default=None, init=False)');
    L.push('');
    ...
    L.push('        parsed = [(spec, STLParser(os.path.join(_OBJECTS, spec[0])).parse()) for spec in OBJECTS]');
    L.push('        with world.modify_world():');
    L.push('            for spec, obj in parsed:');
    L.push('                world.merge_world_at_pose(obj, HomogeneousTransformationMatrix.from_xyz_rpy(');
    L.push('                    spec[1], spec[2], spec[3], yaw=spec[4], reference_frame=world.root))');
    L.push('        for spec, _obj in parsed:');
    L.push('            world.get_body_by_name(spec[0]).visual.shapes[0].color = Color(*spec[5])');
    L.push('');
    L.push('    def build_context(self, world: World) -> Context:');
    L.push('        with world.modify_world():');
    L.push('            WorldReasoner(world).reason()');
    L.push('        robot = self.used_robot.from_world(world)');
    L.push('        ros_node = self.visualization.ros_node if self.visualization is not None else self.ros_node');
    L.push('        context = Context(world=world, robot=robot, _debug=False, ros_node=ros_node)');
    L.push('        context.evaluate_conditions = False');
    ...
    L.push('    """Run the demonstration under the cramera viewer (so the live bridge sees it)."""');
    L.push('    demonstration = ' + cls + '(used_robot=PR2)');
    L.push('    world = demonstration.build_simulated_world()');
    L.push('    demonstration.visualization = WorldVisualization.from_environment(');
    L.push('        world, default_backend=VisualizationBackend.CRAMERA).start()');
    L.push('    if not demonstration.is_scene_populated(world):');
    L.push('        demonstration.populate_scene(world)');
    L.push('    context = demonstration.build_context(world)');
    L.push('    plan = demonstration.build_plan(context)');
    L.push('    demonstration.visualization.attach_plan(plan)');
    L.push('    with simulated_robot:');
    L.push('        plan.perform()');
    L.push('');
    L.push('');
  ...
  $('pb-style').addEventListener('change', function () {
